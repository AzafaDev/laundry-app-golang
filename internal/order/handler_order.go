...
// GetOutletOrderDetail returns a single order detail for outlet admin (staff-facing),
// scoped to the outlet_id instead of customer_id. Same data as GetOrderDetail
// (items, breakdown, status history, payment, complaints) but authorized
// for outlet admins only.
func (h *Handler) GetOutletOrderDetail(c *gin.Context) {
	outletID, hasOutlet := apphelper.CurrentEmployeeOutletID(c)
	if !hasOutlet {
		apperr.RespondError(c, http.StatusForbidden, "no_outlet_assigned")
		return
	...
	ord, err := h.Queries.GetOutletOrderDetail(c.Request.Context(), db.GetOutletOrderDetailParams{
		ID:       orderID,
		OutletID: outletID,
	})
	if errors.Is(err, pgx.ErrNoRows) {
	...
	resp := OrderDetailResponse{
		OrderResponse: toOrderResponseFromOutletDetailRow(ord),
		Items:         make([]OrderItemResponse, 0, len(items)),
		Breakdown:     make([]BreakdownResponse, 0, len(breakdown)),
		StatusHistory: make([]StatusHistoryResponse, 0, len(statusHistory)),
		Payment:       paymentResp,
...
func toOrderResponseFromOutletDetailRow(o db.GetOutletOrderDetailRow) OrderResponse {
	return OrderResponse{
		ID:              o.ID.String(),
		InvoiceNumber:   o.InvoiceNumber,
		OutletID:        o.OutletID.String(),
